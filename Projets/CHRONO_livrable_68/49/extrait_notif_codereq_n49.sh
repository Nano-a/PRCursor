#!/usr/bin/env bash
# CHRONO N°49 — notifs 18–23 (échantillon 18,22) + erreur 31 (adapté de PRCursor/tests/notif_codereq.sh)
set -euo pipefail
...
python3 "$PY" udp "$UDP2" 2.5 > /tmp/n22.out &
R22=$!
sleep 0.2
...
python3 "$PY" mcast "$MCAST" "$MPORT" 2.5 > /tmp/n18.out &
R18=$!
sleep 0.25
...
python3 "$PY" tcp_bad "$H" "$P" > /tmp/n31.out
