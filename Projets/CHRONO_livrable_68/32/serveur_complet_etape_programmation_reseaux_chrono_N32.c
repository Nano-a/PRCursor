/*
 * Snapshot CHRONO N°32 — fonctionnels **POST** / **REPLY** (CHRONO **24–25**) ;
 * pile fonctionnelle identique au **31** ; entête mise sur `handle_post` / `handle_reply`.
 */
...
static int serve_one_codereq(int cfd, struct sockaddr_in6 *peer)
{
    ...
    if (code == PAROLES_CODEREQ_NEW_GROUP) {
        ...
        if (readn(cfd, bodydyn + 2, glen, 30000) < 0) {
            free(bodydyn);
            return -1;
        }
    ...
    if (code == PAROLES_CODEREQ_INVITE) {
        ...
        if (readn(cfd, head, sizeof head, 30000) < 0)
            return -1;
        ...
        if (!ib)
            return -1;
        memcpy(ib, head + 4, 8);
        if (tailsz && readn(cfd, ib + 8, tailsz, 30000) < 0) {
            free(ib);
            return -1;
        }
    ...
    if (code == PAROLES_CODEREQ_LIST_MEM) {
        unsigned char u8[8];
        if (readn(cfd, u8, sizeof u8, 30000) < 0)
            return -1;
    ...
    if (code == PAROLES_CODEREQ_POST) {
        unsigned char h[10];
        if (readn(cfd, h, sizeof h, 30000) < 0)
            return -1;
        ...
        if (dlen) {
            ...
            if (!pay)
                return -1;
            if (readn(cfd, pay, (size_t)dlen, 30000) < 0) {
                free(pay);
                return -1;
            }
        }
    ...
    if (code == PAROLES_CODEREQ_REPLY) {
        ...
        if (rl) {
            ...
            if (!pr)
                return -1;
            if (readn(cfd, pr, (size_t)rl, 30000) < 0) {
                free(pr);
                return -1;
            }
        }
    ...
    if (code == PAROLES_CODEREQ_FEED) {
        ...
        if (readn(cfd, ufeed, sizeof ufeed, 30000) < 0)
            return -1;
