/*
 * Snapshot CHRONO N°23 — dossier **22** + `cmd_listmem` (LIST_MEM 10 / LIST_MEM_OK 11).
 */
...
int main(int argc, char **argv)
{
    ...
    if (argc < 3) {
        fprintf(stderr,
                "usage: %s <host> <port> reg <pseudo>|newgroup <uid> <nom>|invite ...|listinv "
                "<uid>|listmem <uid> <idg>|ans <uid> <idg> <0|1|2>\n",
                argv[0]);
        return 1;
    }
