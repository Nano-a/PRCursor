/*
 * Snapshot CHRONO N°13–16 pour dépôt type programmation_reseaux (TCP nu, sans TLS).
 * À copier en remplacement complet de src/server.c après les commits 14–15.
 *
 * Référence tableau : ligne 16 — formats CODEREQ 3–4 + création groupe côté fil.
 */
...
static int handle_new_group_chrono16(int cfd, uint32_t uid, const unsigned char *body, size_t blen)
{
    ...
    /* Adresse multicast distincte avec idg (aligné étape projet type PRCursor). */
    {
    ...
    unsigned char out[256];
    unsigned char *p = out;
    /* extrait_codereq_3_4 (TCP : writen à la place de conn_writen) */
...
static int serve_one_codereq(int cfd, struct sockaddr_in6 *peer)
{
    ...
    if (code == PAROLES_CODEREQ_NEW_GROUP) {
        ...
        int r = handle_new_group_chrono16(cfd, uid, bodydyn, bodysz);
        free(bodydyn);
        return r;
    }
#endif
...
int main(int argc, char **argv)
{
    ...
    for (;;) {
        ...
        /*
         * N°14 (fin connexion après une requête) : équivalent livrable
         * extrait_server_serve_client_reg_fin — ici généralisé : une commande puis close.
         */
