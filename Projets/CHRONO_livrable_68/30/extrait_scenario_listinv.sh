#!/usr/bin/env bash
# Scénario — après invite : listinv pour l’invité (CHRONO N°30)
set -euo pipefail
H=::1
P="${PORT:-4242}"
...
./paroles_client "$H" "$P" listinv "$U2" | tee /tmp/pc_li.txt
grep -q idg= /tmp/pc_li.txt
echo OK listinv_contains_idg uid_invite="$U2"
