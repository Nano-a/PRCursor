/*
 * Snapshot CHRONO N°13–19 — basé sur le fichier complet du dossier **18**.
 * Dossier 19 : extrait_server_invite_ack.c (send_ack + handle_invite) en TCP nu.
 * + tableaux pending + notif UDP (socket sendto) comme dans la ref PRCursor.
 */
...
static Group *find_group(uint32_t idg) {
    for (int i = 0; i < MAX_GROUPS; i++)
        if (groups[i].used && groups[i].idg == idg)
            return &groups[i];
    return NULL;
}
...
static int serve_one_codereq(int cfd, struct sockaddr_in6 *peer)
{
    ...
    if (code == PAROLES_CODEREQ_INVITE) {
        ...
        uint32_t admin, idg, nb;
        if (wire_get_u32_be(&qp, &lz, &admin) < 0 ||
            wire_get_u32_be(&qp, &lz, &idg) < 0 ||
            wire_get_u32_be(&qp, &lz, &nb) < 0)
            return -1;

        if (nb > MAX_INV_PER_MSG)
            return -1;
        size_t tailsz = (size_t)nb * (4u + PAROLES_INV_PAD);
        ...
        int r = handle_invite(cfd, admin, ib, (size_t)8u + tailsz);
        free(ib);
        return r;
    }
#endif /* !PAROLES_ACCEPT_REAL_CLE_113 */
