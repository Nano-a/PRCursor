/*
 * Snapshot CHRONO N°13–17 (serveur TCP nu, sans TLS) — dossier livrable **17**.
 * Base = même code que dossier 16 (`serveur_complet_etape_programmation_reseaux_chrono_N16.c`) ;
 * différence visible : bloc de commentaires **CHRONO N°17** relié au fichier
 * `extrait_server_idg_et_nom.c` (pas d’unicité sur le nom, idg attribués par `next_gid++`).
 *
 * Copier comme `programmation_reseaux/src/server.c` après le commit des lignes 14–15.
 */
...
/* --- CHRONO N°17 : extrait_server_idg_et_nom.c — premier ID groupe = next_gid après init à 1 --- */
...
static int handle_new_group_tcp(int cfd, uint32_t uid, const unsigned char *body, size_t blen)
{
    ...
    g->used = 1;
    /*
     * --- CHRONO N°17 (suite extrait_server_idg_et_nom.c) ---
     * g->idg = next_gid++ : identifiants groupe toujours > 0 et jamais réutilisés.
     * Deux créations peuvent avoir le même texte dans le nom sans collision d’IDG.
     */
    g->idg = next_gid++;
    ...
    /* Présent dès dossier N°16 (multicast pour réponse CODEREQ 4) ; raffiné ligne 18 tableau. */
    {
...
static int serve_one_codereq(int cfd, struct sockaddr_in6 *peer)
{
    ...
    if (code == PAROLES_CODEREQ_NEW_GROUP) {
        ...
        int r = handle_new_group_tcp(cfd, uid, bodydyn, bodysz);
        free(bodydyn);
        return r;
    }
#endif
