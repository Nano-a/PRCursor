/*
 * Snapshot CHRONO N°22 — base **20** + **21** (INV_ANS…) ; tâche **22** :
 * la règle admin + `ans … 2` → fermeture groupe est déjà dans le serveur snapshot.
 */
...
int main(int argc, char **argv)
{
    ...
    if (argc < 3) {
        fprintf(stderr,
                "usage: %s <host> <port> reg ...|newgroup ...|invite ...|listinv <uid>|ans "
                "<uid> <idg> <0|1|2>\n",
                argv[0]);
        return 1;
    }
